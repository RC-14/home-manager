...
local save = ya.sync(function(st, url, new)
	...
	if hovered and hovered.url == url then
...
local move = ya.sync(function(st)
	...
	if st.last ~= hovered.url then
		st.last, st.level = Url(hovered.url), 0
	end

	return { url = hovered.url, level = st.level }
end)
...
local function canvas(area)
	local cell_width, cell_height = rt.term.cell_size()
	if not cell_width then
		return rt.preview.max_width, rt.preview.max_height
	end

	return math.min(rt.preview.max_width, math.floor(area.w * cell_width)),
		math.min(rt.preview.max_height, math.floor(area.h * cell_height))
end
...
local function peek(_, job)
	...
	local max_width, max_height = canvas(job.area)
	local min_width, min_height = math.min(max_width, info.w), math.min(max_height, info.h)
	local new_width = min_width + math.floor(min_width * level * 0.1)
	local new_height = min_height + math.floor(min_height * level * 0.1)
	if new_width > max_width or new_height > max_height then
		if job.old_level then
			return sync() -- Image larger than available preview area after zooming
		else
			new_width, new_height = max_width, max_height -- Run as a previewer, render the image anyway
		end
	end
	...
	local status, err = Command("magick"):arg {
		tostring(url),
		"-auto-orient", "-strip",
		"-sample", string.format("%dx%d", new_width, new_height),
